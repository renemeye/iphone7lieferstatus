...
(function() {
  var background_color, border_color, colors, database, devices, load_order_date_selector, model_sorter, model_storage, model_type, models, nicer_date, order_dates, pad, prepare_week_table, print_footer, print_header, process_data, process_magenta1, process_with_ticket, process_without_ticket, storages, stringify_model, ticket_dates, types,
  ...
  types = ['iPhone 7', 'iPhone 7 Plus', 'iPhone 6s', 'iPhone 6s Plus'];
  ...
  process_data = function(data, no_ticket_date_value) {
    var info, model, results, shipping_infos;
    if (no_ticket_date_value == null) {
      no_ticket_date_value = '';
    }
    results = [];
    for (model in data) {
      shipping_infos = data[model];
      if (devices[model]) {
      ...
      results.push((function() {
        var base, i, len, name, name1, ref, ref1, results1;
        results1 = [];
        for (i = 0, len = shipping_infos.length; i < len; i++) {
          info = shipping_infos[i];
          info.ticket_date = info.ticket_date || no_ticket_date_value;
          if (ref = info.ticket_date, indexOf.call(ticket_dates, ref) < 0) {
            ticket_dates.push(info.ticket_date);
          }
          if (ref1 = info.order_date, indexOf.call(order_dates, ref1) < 0) {
          ...
          results1.push(database[info.ticket_date][info.order_date][model] = info.shipping_text);
        }
        return results1;
      })());
    }
  ...
  process_with_ticket = function() {
    return process_data(data_with_ticket.data);
  };

  process_without_ticket = function() {
    return process_data(data_without_ticket.data, 'Ohne Ticket');
  };
